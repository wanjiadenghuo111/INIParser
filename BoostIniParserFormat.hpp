                    ...
                    const Ptree& pt)
{
            ...
            it != end; ++it)
    {
        if (!it->second.empty())
        {
            ...
            stream << Ch('\n');//add blank line after one section,let ini read friendly.
        }
    }
}
}
 ...
 */
template<class Ptree>
void write_ini_v1(std::basic_ostream <
                  typename Ptree::key_type::value_type
                  > &stream,
                  const Ptree& pt,
                  int flags = 0)
 ...
 */
template<class Ptree>
void write_ini_v1(const std::string& filename,
                  const Ptree& pt,
                  int flags = 0,
                  const std::locale& loc = std::locale())
{
